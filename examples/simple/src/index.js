...
function setUpReorderableEvents(options) {
  ...
  function handleDragMove(event) {
    ...
    if (
      Math.abs(x - lastClientX) >= distanceToBeginDragging ||
      Math.abs(y - lastClientY) >= distanceToBeginDragging ||
      (event instanceof MouseEvent &&
        lastClientX === null &&
        lastClientY === null) // otherwise, initial hovers are skipped
    ) {
      ...
      if (placeholder !== null) {
        var reorderableElems = document.querySelectorAll(
          reorderableQuerySelector
        );
        var reorderableBounds = []; // List BoundingClientRect

        for (
          var index = 0, length = reorderableElems.length;
          index < length;
          index++
        ) {
          var reorderableElem = reorderableElems[index];

          reorderableBounds.push(reorderableElem.getBoundingClientRect());
        }
...
var Reorderable = {
  setUpReorderableEvents: setUpReorderableEvents,
  setDragAxis: setDragAxis
};
